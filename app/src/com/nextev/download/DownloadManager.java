...
public class DownloadManager extends Thread {
	...
	public void addHandler(Handler handler) {
		DebugLog.v("addHandler() ---> Before mDLHandler.size = " + mDLHandler.size());
		if (!mDLHandler.contains(handler)) {
			mDLHandler.add(handler);
			DebugLog.v("addHandler() ---> After mDLHandler.size = " + mDLHandler.size());
		}
	}

	public void clearHandler(Handler handler) {
		DebugLog.v("clearHandler() ---> Before mDLHandler.size = " + mDLHandler.size());
		if (mDLHandler.contains(handler)) {
			mDLHandler.remove(handler);
			DebugLog.v("clearHandler() ---> After mDLHandler.size = " + mDLHandler.size());
		}
	}
	...
	public List<DLFileInfo> checkUncompleteTasks() {
		...
		List<DLFileInfo> dlApkList = DownloadUtil.getURLArray(mContext);
		if (dlApkList.size() >= 0) {
	...
	public synchronized void deleteTask(DLFileInfo dLFileInfo) {
		...
		DownloadUtil.clearURL(mContext, index);
	}
	...
	public synchronized void completeTask(DownloadTask task) {
		if (mDownloadingTasks.contains(task)) {
			...
			DownloadUtil.storeURL(mContext, index, task.getDLFileInfo());
			mDownloadingTasks.remove(task);
