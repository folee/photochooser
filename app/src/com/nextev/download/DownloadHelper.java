...
public class DownloadHelper {
	...
	/**
	 * 下载连接超时时长设置,默认为3000ms.({ DownloadTask.TIME_OUT})
	 * 
	 * @param time
	 *            下载URL连接超时时长(ms)
	 */
	...
	/**
	 * 下载缓冲区长度设置,默认大小为1024 * 8.({ DownloadTask.BUFFER_SIZE})
	 * 
	 * @param bufferSize
	 *            下载缓冲区大小
	 */
	...
	/**
	 * 是否打开下载DEBUG调试模式，默认打开.({  DownloadTask.DEBUG})
	 * 
	 * @param isDebug
	 */
	...
	/**
	 * 设置下载任务队列长度,默认为100.({ DownloadTask.MAX_TASK_COUNT})
	 * 
	 * @param maxCount
	 *            任务队列最大长度
	 */
	...
	/**
	 * 设置最大允许下载线程同时运行的个数,默认为1.({DownloadTask.MAX_DOWNLOAD_THREAD_COUNT} )
	 * 
	 * @param threadCount
	 *            最大同时下载线程数
	 */
	...
	/**
	 * 下载条件是否满足的监听类.({ DownloadService.onCreate})</br>
